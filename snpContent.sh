...
#grep -P 'missense_variant|#CHROM' $vcffile | sed 's/AC.*\(ANN\)/\1/' | \
sed -e 's/0\/0:[^\t]*/Ref/g' -e 's/0\/1:[^\t]*/Het/g' -e 's/1\/1:[^\t]*/Hom/g' -e 's/\.\/\.:[^\t]*/Mis/g' > ${vcffile%.vcf}.targetSNPcontent.txt
...
n=2
cat ${vcffile%.txt}_snpName.txt | while read snp; do awk '{print "'$snp'\t"$NF"\t"$'$n'}' ${vcffile%.txt}.tr.class.txt; let n++; done > \
${vcffile%.txt}.tr.class.plot.txt
